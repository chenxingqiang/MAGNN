    ...
    def forward(self, inputs):
        ...
        if self.rnn_type == "gru":
            ...
            hidden = (
                ...
                .reshape(-1, self.out_dim, self.num_heads)
                .permute(0, 2, 1)
            ...
            if self.rnn_type == "TransE0":
                ...
                r_vec = r_vec.reshape(
                    self.r_vec.shape[0] * 2, self.r_vec.shape[1]
                )  # etypes x out_dim
            ...
            for i in range(final_r_vec.shape[0] - 2, -1, -1):
                ...
                if self.etypes[i] is not None:
                    final_r_vec[i, :, 0] = (
                        final_r_vec[i + 1, :, 0].clone() * r_vec[self.etypes[i], :, 0]
                        - final_r_vec[i + 1, :, 1].clone() * r_vec[self.etypes[i], :, 1]
                    )
                    final_r_vec[i, :, 1] = (
                        final_r_vec[i + 1, :, 0].clone() * r_vec[self.etypes[i], :, 1]
...
class MAGNN_ctr_ntype_specific(nn.Module):
    def __init__(
        ...
        for i in range(num_metapaths):
            self.metapath_layers.append(
                MAGNN_metapath_specific(
                    etypes_list[i],
                    out_dim,
                    num_heads,
                    rnn_type,
    ...
    def forward(self, inputs):
        if self.use_minibatch:
            g_list, features, type_mask, edge_metapath_indices_list, target_idx_list = (
                inputs
            )
            ...
            metapath_outs = [
                ...
                for g, edge_metapath_indices, target_idx, metapath_layer in zip(
                    g_list,
            ...
            metapath_outs = [
                ...
                for g, edge_metapath_indices, metapath_layer in zip(
                    g_list, edge_metapath_indices_list, self.metapath_layers
                )
            ]
        ...
        beta = torch.unsqueeze(beta, dim=-1)
        metapath_outs = [
            torch.unsqueeze(metapath_out, dim=0) for metapath_out in metapath_outs
        ]
