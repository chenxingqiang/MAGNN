...
lr = 0.005
weight_decay = 0.001
etypes_lists = [[[0, 1], [2, 3]], [[1, 0], [1, 2, 3, 0]], [[3, 2], [3, 0, 1, 2]]]
...
def run_model_IMDB(
    ...
    features_list = [
        torch.FloatTensor(features.todense()).to(device) for features in features_list
    ]
    if feats_type == 0:
        ...
        for nx_G in nx_G_list:
            ...
            g.add_edges(
                *list(
                    zip(
                        *sorted(
                            map(lambda tup: (int(tup[0]), int(tup[1])), nx_G.edges())
                        )
                    )
                )
            )
    ...
    for _ in range(repeat):
        ...
        for epoch in range(num_epochs):
            ...
            logits, embeddings = net(
                (g_lists, features_list, type_mask, edge_metapath_indices_lists),
                target_node_indices,
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--layers", type=int, default=2, help="Number of layers. Default is 2."
    )
    ap.add_argument(
        "--hidden-dim",
        type=int,
    ...
    ap.add_argument(
        "--save-postfix",
        default="IMDB",
        help="Postfix for the saved model and result. Default is IMDB.",
    )
