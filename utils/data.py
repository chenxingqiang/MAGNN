...
def load_LastFM_data(prefix="data/preprocessed/LastFM_processed"):
    ...
    adjM = scipy.sparse.load_npz(prefix + "/adjM.npz")
    type_mask = np.load(prefix + "/node_types.npy")
    train_val_test_pos_user_artist = np.load(
        prefix + "/train_val_test_pos_user_artist.npz"
    )
    train_val_test_neg_user_artist = np.load(
        prefix + "/train_val_test_neg_user_artist.npz"
    )

    return (
        [[adjlist00, adjlist01, adjlist02], [adjlist10, adjlist11, adjlist12]],
...
def load_metapath2vec_embedding(path, type_list, num_embeddings_list, offset_list):
    ...
    with open(path, "r") as infile:
        _, dim = list(map(int, infile.readline().strip().split(" ")))
        embeddings_dict = {
            type: np.zeros((num_embeddings, dim))
            for type, num_embeddings in zip(type_list, num_embeddings_list)
        }
        ...
        for line in infile.readlines():
            ...
            if line[0] == "</s>":
                continue
            count += 1
            embeddings_dict[line[0][0]][int(line[0][1:]) - offset_dict[line[0][0]]] = (
