...
def run_model_DBLP(
    ...
    for _ in range(repeat):
        ...
        val_idx_generator = index_generator(
            batch_size=batch_size, indices=val_idx, shuffle=False
        )
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for iteration in range(val_idx_generator.num_iterations()):
                    ...
                    val_g_list, val_indices_list, val_idx_batch_mapped_list = (
                        parse_minibatch(
                            adjlists,
                            edge_metapath_indices_list,
                            val_idx_batch,
        ...
        test_idx_generator = index_generator(
            batch_size=batch_size, indices=test_idx, shuffle=False
        )
        net.load_state_dict(
        ...
        with torch.no_grad():
            for iteration in range(test_idx_generator.num_iterations()):
                ...
                test_g_list, test_indices_list, test_idx_batch_mapped_list = (
                    parse_minibatch(
                        adjlists,
                        edge_metapath_indices_list,
                        test_idx_batch,
    ...
    print(
        ...
        + ", ".join(
            [
                "{:.6f}~{:.6f} ({:.1f})".format(
                    macro_f1[:, 0].mean(), macro_f1[:, 1].mean(), train_size
                )
                for macro_f1, train_size in zip(
                    svm_macro_f1_lists, [0.8, 0.6, 0.4, 0.2]
                )
            ]
        )
    )
    print(
        ...
        + ", ".join(
            [
                "{:.6f}~{:.6f} ({:.1f})".format(
                    micro_f1[:, 0].mean(), micro_f1[:, 1].mean(), train_size
                )
                for micro_f1, train_size in zip(
                    svm_micro_f1_lists, [0.8, 0.6, 0.4, 0.2]
                )
            ]
        )
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--epoch", type=int, default=100, help="Number of epochs. Default is 100."
    )
    ap.add_argument("--patience", type=int, default=5, help="Patience. Default is 5.")
    ap.add_argument(
        "--batch-size", type=int, default=8, help="Batch size. Default is 8."
    )
    ap.add_argument(
        "--samples",
        type=int,
    ...
    ap.add_argument(
        "--save-postfix",
        default="DBLP",
        help="Postfix for the saved model and result. Default is DBLP.",
    )
